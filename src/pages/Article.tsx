      ...
      <main id="main-content" className="container-blog py-8 max-w-4xl">
        ...
        <div className="space-y-4 mb-6">
          <div className="flex flex-wrap gap-2">
            {post.categories && (
              <Link to={`/category/${post.categories.slug}`} className="badge-category">{post.categories.name}</Link>
            )}
            {extraCats.map(c => (
              <Link key={c.id} to={`/category/${c.slug}`} className="badge-category opacity-80 hover:opacity-100">{c.name}</Link>
            ))}
          </div>
        ...
        <div className="prose prose-lg max-w-none dark:prose-invert mb-8" dangerouslySetInnerHTML={{ __html: sanitizeHtml(post.content) }} />
