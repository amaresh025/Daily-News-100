...
// ../.wrangler/tmp/bundle-Zs7pXN/checked-fetch.js
...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  const getJwtProjectRef = /* @__PURE__ */ __name((token) => {
  ...
  try {
    const { data: category, error: catErr } = await supabase.from("categories").select("id").eq("slug", "ai").maybeSingle();
    if (catErr) {
      return new Response(
    ...
    const { data: posts, error: postsErr } = await supabase.from("posts").select("id, title, excerpt, featured_image, slug, published_at").eq("status", "published").or(`category_id.eq.${category.id},extra_category_ids.cs.{${category.id}}`).lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).order("published_at", { ascending: false }).limit(limit);
    if (postsErr) {
      return new Response(
    ...
    const formattedPosts = (posts || []).map((post2) => ({
      id: post2.id,
      title: post2.title,
      excerpt: post2.excerpt || "",
      image: post2.featured_image || "",
      slug: post2.slug,
      publishedAt: post2.published_at || "",
      url: `https://dailynews100.com/blog/${post2.slug}`
    }));
    return new Response(
...
var onRequest2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    const { data: post2, error } = await supabase.from("posts").select("id").eq("slug", slug).eq("status", "published").lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).maybeSingle();
    if (error || !post2) {
      return new Response(
        `<!DOCTYPE html>
<html lang="en">
...
var onRequest4 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    const { data: category, error } = await supabase.from("categories").select("id").eq("slug", slug).maybeSingle();
    if (error || !category) {
      return new Response(
        `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Category not found | DailyNews100</title>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@400;500;700;800&display=swap" rel="stylesheet">
    <style>
...
function formatIsoDate(dateStr, fallbackDate = "2026-07-13T00:00:00Z") {
...
async function generateNewsSitemap(supabase, domain) {
  const fortyEightHoursAgo = new Date(Date.now() - 48 * 60 * 60 * 1e3).toISOString();
  const { data: posts, error } = await supabase.from("posts").select("title, slug, published_at").eq("status", "published").lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).gte("published_at", fortyEightHoursAgo).order("published_at", { ascending: false }).limit(1e3);
  if (error) {
...
`;
  if (posts) {
    for (const post2 of posts) {
      const pubDate = formatIsoDate(post2.published_at);
      xml += `  <url>
`;
      xml += `    <loc>${domain}/blog/${escapeXml(post2.slug)}</loc>
`;
      xml += `    <news:news>
`;
...
`;
      xml += `      <news:title>${escapeXml(post2.title)}</news:title>
`;
      xml += `    </news:news>
`;
      xml += `  </url>
...
__name(generateNewsSitemap, "generateNewsSitemap");
var onRequest5 = /* @__PURE__ */ __name(async (context) => {
  const sitemapParam = context.params.sitemap;
  const sitemapRegex = /^(sitemap|news-sitemap)(?:-index|-(\d+))?\.xml$/;
  const match2 = sitemapParam.match(sitemapRegex);
  if (!match2) {
    const url = new URL(context.request.url);
  ...
  try {
    ...
    const { count: articleCount, error: countErr } = await supabase.from("posts").select("*", { count: "exact", head: true }).eq("status", "published").lte("published_at", (/* @__PURE__ */ new Date()).toISOString());
    if (countErr) {
      return new Response(`Error querying posts count: ${JSON.stringify(countErr)}`, { status: 500 });
    }
    const { data: categories, error: catErr } = await supabase.from("categories").select("slug, updated_at, created_at");
    if (catErr) {
    ...
    const needsIndex = totalUrls > 5e4;
    const { data: latestPost } = await supabase.from("posts").select("updated_at, published_at").eq("status", "published").lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).order("updated_at", { ascending: false }).limit(1).maybeSingle();
...
`;
      xml2 += `<sitemapindex xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
`;
      for (let i = 1; i <= totalSitemaps; i++) {
        xml2 += `  <sitemap>
`;
        xml2 += `    <loc>${domain}/sitemap-${i}.xml</loc>
`;
        xml2 += `    <lastmod>${formatIsoDate(latestDbDate, defaultFallbackDate)}</lastmod>
`;
        xml2 += `    </sitemap>
`;
      }
...
`;
    ...
    if (articleStart < totalArticles) {
      const { data: posts, error: postsErr } = await supabase.from("posts").select("slug, updated_at, published_at").eq("status", "published").lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).order("published_at", { ascending: false }).range(articleStart, articleEnd);
      if (postsErr) {
      ...
      if (posts) {
        for (const post2 of posts) {
          urls2.push({
            loc: `${domain}/blog/${post2.slug}`,
            lastmod: formatIsoDate(post2.updated_at || post2.published_at, latestDbDate),
            changefreq: "weekly",
            priority: "0.7"
          });
        }
...
// ../.wrangler/tmp/pages-FzQSGJ/functionsRoutes-0.9143314271663444.mjs
var routes = [
  {
...
// C:/Users/Amresh/AppData/Roaming/npm/node_modules/wrangler/node_modules/path-to-regexp/dist.es2015/index.js
function lexer(str) {
  var tokens = [];
  var i = 0;
  while (i < str.length) {
...
// C:/Users/Amresh/AppData/Roaming/npm/node_modules/wrangler/templates/pages-template-worker.ts
var escapeRegex = /[.+?^${}()|[\]\\]/g;
...
// C:/Users/Amresh/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-ensure-req-body-drained.ts
...
// C:/Users/Amresh/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/middleware-miniflare3-json-error.ts
...
// ../.wrangler/tmp/bundle-Zs7pXN/middleware-insertion-facade.js
...
// C:/Users/Amresh/AppData/Roaming/npm/node_modules/wrangler/templates/middleware/common.ts
...
// ../.wrangler/tmp/bundle-Zs7pXN/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9876216764547208.mjs.map
