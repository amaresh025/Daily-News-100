...
// ../.wrangler/tmp/bundle-BHpDrQ/checked-fetch.js
...
var onRequest = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const { data: posts, error: postsErr } = await supabase.from("posts").select("id, title, excerpt, featured_image, slug, published_at").eq("status", "published").or(`category_id.eq.${category.id},extra_category_ids.cs.{${category.id}}`).lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).order("published_at", { ascending: false }).limit(6);
    if (postsErr) {
      return new Response(
...
var onRequest2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    ...
    const { data: latestPost } = await supabase.from("posts").select("updated_at, created_at").eq("status", "published").lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).order("published_at", { ascending: false }).limit(1).maybeSingle();
    const fallbackDate = (/* @__PURE__ */ new Date()).toISOString();
...
`;
        xml2 += `    <lastmod>${formatIsoDate(latestDate)}</lastmod>
`;
        xml2 += `  </sitemap>
`;
      }
...
`;
    ...
    if (articleStart < totalArticles) {
      const { data: posts, error: postsErr } = await supabase.from("posts").select("slug, updated_at, created_at").eq("status", "published").lte("published_at", (/* @__PURE__ */ new Date()).toISOString()).order("published_at", { ascending: false }).range(articleStart, articleEnd);
      if (postsErr) {
      ...
      if (posts) {
        for (const post2 of posts) {
          urls2.push({
            ...
            lastmod: formatIsoDate(post2.updated_at || post2.created_at),
            changefreq: "hourly",
            priority: "0.9"
          });
        }
...
// ../.wrangler/tmp/pages-IPMawg/functionsRoutes-0.0034935542415938103.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-BHpDrQ/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-BHpDrQ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.46561006833702945.mjs.map
